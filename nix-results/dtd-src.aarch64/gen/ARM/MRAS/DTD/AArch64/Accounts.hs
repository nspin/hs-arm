module ARM.MRAS.DTD.AArch64.Accounts where
...
data Encoding = Encoding{encodingEncname :: String}
              deriving (Eq, Show)

newtype Encodings = Encodings (List1 Encoding)
                  deriving (Eq, Show)

newtype Orig = Orig String
             deriving (Eq, Show)

newtype Intro = Intro String
              deriving (Eq, Show)
...
data Account_Attrs = Account_Attrs{accountIclass :: String,
                                   accountSymbol :: String, accountIclass_long :: String}
                   deriving (Eq, Show)

newtype Accounts = Accounts (List1 Account)
                 deriving (Eq, Show)
...
instance XmlContent Encoding where
        toContents as = [CElem (Elem (N "encoding") (toAttrs as) []) ()]
        parseContents
          = do (Elem _ as []) <- element ["encoding"]
               return (fromAttrs as)
              `adjustErr` ("in <encoding>, " ++)

instance XmlAttributes Encoding where
        fromAttrs as
          = Encoding{encodingEncname =
                       definiteA fromAttrToStr "encoding" "encname" as}
        toAttrs v = catMaybes [toAttrFrStr "encname" (encodingEncname v)]
...
instance XmlContent Encodings where
        ...
        parseContents
          = do e@(Elem _ [] _) <- element ["encodings"]
...
instance XmlContent Orig where
        toContents (Orig a) = [CElem (Elem (N "orig") [] (toText a)) ()]
        parseContents
          = do e@(Elem _ [] _) <- element ["orig"]
...
instance XmlContent Intro where
        toContents (Intro a) = [CElem (Elem (N "intro") [] (toText a)) ()]
        parseContents
...
instance XmlContent Account where
        ...
        parseContents
          = do e@(Elem _ as _) <- element ["account"]
               interior e $
                 return (Account (fromAttrs as)) `apply` parseContents `apply`
                   parseContents
                   `apply` parseContents
...
instance XmlAttributes Account_Attrs where
        fromAttrs as
          = Account_Attrs{accountIclass =
                            definiteA fromAttrToStr "account" "iclass" as,
                          accountSymbol = definiteA fromAttrToStr "account" "symbol" as,
                          accountIclass_long =
                            definiteA fromAttrToStr "account" "iclass_long" as}
        toAttrs v
          = catMaybes
...
instance XmlContent Accounts where
        ...
        parseContents
          = do e@(Elem _ [] _) <- element ["accounts"]
