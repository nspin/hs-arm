module ARM.MRAS.DTD.AArch64.Permindex where
...
data Line_Attrs = Line_Attrs{lineInitialtag :: (Maybe String)}
                deriving (Eq, Show)

data In = In In_Attrs String
        deriving (Eq, Show)

data In_Attrs = In_Attrs{inLink :: String}
              deriving (Eq, Show)

data Initials = Initials Initials_Attrs (List1 In)
              deriving (Eq, Show)

data Initials_Attrs = Initials_Attrs{initialsHowmany :: String}
                    deriving (Eq, Show)
...
data Permindex_Attrs = Permindex_Attrs{permindexTitlename ::
                                       String,
...
instance XmlContent Line where
        toContents (Line as a b)
          = [CElem
               (Elem (N "line") (toAttrs as) (toContents a ++ toContents b))
               ()]
        parseContents
          = do e@(Elem _ as _) <- element ["line"]
               interior e $
                 return (Line (fromAttrs as)) `apply` parseContents `apply`
                   parseContents
              `adjustErr` ("in <line>, " ++)

instance XmlAttributes Line_Attrs where
        fromAttrs as
          = Line_Attrs{lineInitialtag =
                         possibleA fromAttrToStr "initialtag" as}
        toAttrs v
          = catMaybes
...
instance XmlContent In where
        ...
        parseContents
          = do e@(Elem _ as _) <- element ["in"]
               interior e $
                 return (In (fromAttrs as)) `apply` (text `onFail` return "")
              `adjustErr` ("in <in>, " ++)

instance XmlAttributes In_Attrs where
        fromAttrs as
          = In_Attrs{inLink = definiteA fromAttrToStr "in" "link" as}
        toAttrs v = catMaybes [toAttrFrStr "link" (inLink v)]
...
instance XmlContent Initials where
        ...
        parseContents
          = do e@(Elem _ as _) <- element ["initials"]
               interior e $ return (Initials (fromAttrs as)) `apply` parseContents
              `adjustErr` ("in <initials>, " ++)

instance XmlAttributes Initials_Attrs where
        fromAttrs as
          = Initials_Attrs{initialsHowmany =
                             definiteA fromAttrToStr "initials" "howmany" as}
        toAttrs v = catMaybes [toAttrFrStr "howmany" (initialsHowmany v)]
...
instance XmlContent Permindex where
        toContents (Permindex as a b)
          = [CElem
               (Elem (N "permindex") (toAttrs as)
                  (maybe [] toContents a ++ toContents b))
               ()]
        parseContents
          = do e@(Elem _ as _) <- element ["permindex"]
               interior e $
                 return (Permindex (fromAttrs as)) `apply` optional parseContents
                   `apply` parseContents
              `adjustErr` ("in <permindex>, " ++)

instance XmlAttributes Permindex_Attrs where
        fromAttrs as
          = Permindex_Attrs{permindexTitlename =
                              definiteA fromAttrToStr "permindex" "titlename" as,
