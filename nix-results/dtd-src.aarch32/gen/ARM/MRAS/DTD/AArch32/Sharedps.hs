module ARM.MRAS.DTD.AArch32.Sharedps where
...
newtype Aml_para = Aml_para [Aml_para_] 		deriving (Eq,Show)
...
newtype Aml_bullets = Aml_bullets [Aml_bullets_] 		deriving (Eq,Show)

data Aml_bullets_ = Aml_bullets_Aml_bullets Aml_bullets
                  | Aml_bullets_Aml_bullet Aml_bullet
                  deriving (Eq,Show)

newtype Aml_bullet = Aml_bullet [Aml_bullet_] 		deriving (Eq,Show)
...
newtype Desc = Desc Brief 		deriving (Eq,Show)

newtype Brief = Brief [Brief_] 		deriving (Eq,Show)
...
newtype Psheading = Psheading [Psheading_] 		deriving (Eq,Show)
...
data Codegroup = Codegroup Codegroup_Attrs (List1 Code)
               deriving (Eq,Show)
data Codegroup_Attrs = Codegroup_Attrs
    { codegroupId :: (Maybe String)
    , codegroupMylink :: (Maybe String)
    , codegroupHeading :: (Maybe String)
    , codegroupFile :: String
...
data Code_Attrs = Code_Attrs
    { codeSection :: (Maybe String)
    , codeHeading :: (Maybe String)
    } deriving (Eq,Show)

newtype Codeintro = Codeintro [Codeintro_] 		deriving (Eq,Show)
...
data Asl_line = Asl_line Asl_line_Attrs [Asl_line_]
              deriving (Eq,Show)
data Asl_line_Attrs = Asl_line_Attrs
    { asl_lineFst :: (Maybe Asl_line_fst)
...
newtype Amltext = Amltext String 		deriving (Eq,Show)
...
instance XmlContent Aml_bullets where
    ...
    parseContents = do
        { e@(Elem _ [] _) <- element ["aml-bullets"]
...
instance XmlContent Ps_section where
    ...
    parseContents = do 
        ...
        ; interior e $ oneOf
            ...
            , return (Ps_sectionPsheading_Codegroup (fromAttrs as))
              `apply` parseContents
...
instance XmlContent Ps where
    toContents (Ps as a b) =
        [CElem (Elem (N "ps") (toAttrs as) (maybe [] toContents a ++
                                            maybe [] toContents b)) ()]
    parseContents = do
        { e@(Elem _ as _) <- element ["ps"]
        ; interior e $ return (Ps (fromAttrs as))
...
instance XmlAttrType Instructionsection_type where
    fromAttrToTyp n (N n',v)
        ...
        | otherwise = Nothing
      where translate "pseudocode" = Just Instructionsection_type_pseudocode
            translate _ = Nothing
...
instance XmlContent Codegroup where
    ...
    parseContents = do
        { e@(Elem _ as _) <- element ["codegroup"]
        ; interior e $ return (Codegroup (fromAttrs as))
                       `apply` parseContents
        } `adjustErr` ("in <codegroup>, "++)
instance XmlAttributes Codegroup_Attrs where
    fromAttrs as =
        Codegroup_Attrs
          { codegroupId = possibleA fromAttrToStr "id" as
...
instance XmlContent Code where
    toContents (Code as a b) =
        [CElem (Elem (N "code") (toAttrs as) (maybe [] toContents a ++
                                              toContents b)) ()]
    parseContents = do
        { e@(Elem _ as _) <- element ["code"]
        ; interior e $ return (Code (fromAttrs as))
                       `apply` optional parseContents `apply` parseContents
        } `adjustErr` ("in <code>, "++)
instance XmlAttributes Code_Attrs where
    fromAttrs as =
        Code_Attrs
          { codeSection = possibleA fromAttrToStr "section" as
...
instance XmlContent Asl_line where
    toContents (Asl_line as a) =
        [CElem (Elem (N "asl_line") (toAttrs as) (concatMap toContents a)) ()]
    parseContents = do
        { e@(Elem _ as _) <- element ["asl_line"]
        ; interior e $ return (Asl_line (fromAttrs as))
                       `apply` many parseContents
        } `adjustErr` ("in <asl_line>, "++)
instance XmlAttributes Asl_line_Attrs where
    fromAttrs as =
        Asl_line_Attrs
          { asl_lineFst = possibleA fromAttrToTyp "fst" as
