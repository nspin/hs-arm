module ARM.MRAS.DTD.AArch32.Iformp where
...
data Instruction_Attrs = Instruction_Attrs{instructionParam0 ::
                                           (Maybe String)}
                       deriving (Eq, Show)
...
data Xref_Attrs = Xref_Attrs{xrefLinkend :: String}
                deriving (Eq, Show)
...
data Brief_Attrs = Brief_Attrs{briefSynth :: (Maybe Brief_synth),
...
newtype Authored = Authored [Authored_]
                 deriving (Eq, Show)
...
newtype Encodingnotes = Encodingnotes [Encodingnotes_]
                      deriving (Eq, Show)
...
newtype Syntaxnotes = Syntaxnotes [Syntaxnotes_]
                    deriving (Eq, Show)
...
data Asmtemplate_Attrs = Asmtemplate_Attrs{asmtemplateRole ::
                                           (Maybe Asmtemplate_role),
...
data Aliasmnem_Attrs = Aliasmnem_Attrs{aliasmnemMnemonic :: String,
                                       aliasmnemId :: String, aliasmnemIformqual :: (Maybe String),
...
data Alias_Attrs = Alias_Attrs{aliasEncname :: String,
                               aliasEquivalent_to :: String, aliasAssembler_prototype :: String,
                               aliasEnctag :: (Maybe String), aliasConditions :: (Maybe String),
                               aliasDescription :: (Maybe String)}
                 deriving (Eq, Show)

newtype Conditionexpr = Conditionexpr Pstext
                      deriving (Eq, Show)
...
data Box_Attrs = Box_Attrs{boxHibit :: String,
                           boxName :: (Maybe String), boxWidth :: (Maybe String),
                           boxUsename :: (Maybe Box_usename), boxSettings :: (Maybe String),
                           boxPsbits :: (Maybe String), boxConstraint :: (Maybe String),
...
data Arch_variant = Arch_variant{arch_variantName ::
                                 (Maybe String),
...
data Table_Attrs = Table_Attrs{tableClass :: Table_class}
                 deriving (Eq, Show)
...
data Entry_Attrs = Entry_Attrs{entryClass :: Entry_class,
                               entryBitwidth :: (Maybe String), entryIclasslink :: (Maybe String),
...
data Entry_class = Entry_class_symbol
                 | Entry_class_bitfield
                 | Entry_class_feature
                 deriving (Eq, Show)
...
data Constrained_unpredictables_Attrs = Constrained_unpredictables_Attrs{constrained_unpredictablesEncoding
                                                                         :: (Maybe String),
                                                                         constrained_unpredictablesPs_block
                                                                         :: (Maybe String)}
                                      deriving (Eq, Show)
...
newtype Cu_cause = Cu_cause Pstext
                 deriving (Eq, Show)
...
data Cu_type_Attrs = Cu_type_Attrs{cu_typeConstraint ::
                                   (Maybe String)}
                   deriving (Eq, Show)

data Cu_type_variable = Cu_type_variable{cu_type_variableName ::
                                         String,
...
data Image = Image{imageFile :: String,
...
instance XmlContent Instruction where
        toContents (Instruction as a)
          = [CElem
               (Elem (N "instruction") (toAttrs as) (concatMap toContents a))
               ()]
        parseContents
          = do e@(Elem _ as _) <- element ["instruction"]
               interior e $
                 return (Instruction (fromAttrs as)) `apply` many parseContents
              `adjustErr` ("in <instruction>, " ++)

instance XmlAttributes Instruction_Attrs where
        fromAttrs as
          = Instruction_Attrs{instructionParam0 =
                                possibleA fromAttrToStr "param0" as}
        toAttrs v
          = catMaybes
...
instance XmlContent Parameter where
        ...
        parseContents
          = do e@(Elem _ [] _) <- element ["parameter"]
...
instance XmlContent Xref where
        toContents (Xref as a)
          = [CElem (Elem (N "xref") (toAttrs as) (concatMap toContents a))
               ()]
        parseContents
          = do e@(Elem _ as _) <- element ["xref"]
               interior e $
                 return (Xref (fromAttrs as)) `apply` many parseContents
              `adjustErr` ("in <xref>, " ++)

instance XmlAttributes Xref_Attrs where
        fromAttrs as
          = Xref_Attrs{xrefLinkend =
                         definiteA fromAttrToStr "xref" "linkend" as}
        toAttrs v = catMaybes [toAttrFrStr "linkend" (xrefLinkend v)]
...
instance XmlAttributes Brief_Attrs where
        fromAttrs as
          = Brief_Attrs{briefSynth = possibleA fromAttrToTyp "synth" as,
...
instance XmlContent Alg where
        ...
        parseContents
          = do e@(Elem _ as _) <- element ["alg"]
               interior e $
                 return (Alg (fromAttrs as)) `apply` (text `onFail` return "")
              `adjustErr` ("in <alg>, " ++)
...
instance XmlContent Authored where
        ...
        parseContents
          = do e@(Elem _ [] _) <- element ["authored"]
...
instance XmlContent Encodingnotes where
        ...
        parseContents
          = do e@(Elem _ [] _) <- element ["encodingnotes"]
...
instance XmlContent Syntaxnotes where
        ...
        parseContents
          = do e@(Elem _ [] _) <- element ["syntaxnotes"]
...
instance XmlAttributes Asmtemplate_Attrs where
        fromAttrs as
          = Asmtemplate_Attrs{asmtemplateRole =
                                possibleA fromAttrToTyp "role" as,
...
instance XmlContent Alias where
        toContents (Alias as a b c d)
          = [CElem
               (Elem (N "alias") (toAttrs as)
                  (maybe [] toContents a ++
                     toContents b ++ toContents c ++ maybe [] toContents d))
               ()]
        parseContents
          = do e@(Elem _ as _) <- element ["alias"]
               interior e $
                 return (Alias (fromAttrs as)) `apply` optional parseContents
                   `apply` parseContents
                   `apply` parseContents
                   `apply` optional parseContents
              `adjustErr` ("in <alias>, " ++)
...
instance XmlAttributes Alias_Attrs where
        fromAttrs as
          = Alias_Attrs{aliasEncname =
                          definiteA fromAttrToStr "alias" "encname" as,
                        aliasEquivalent_to =
...
instance XmlContent Conditionexpr where
        ...
        parseContents
          = do e@(Elem _ [] _) <- element ["conditionexpr"]
...
instance XmlAttributes C_Attrs where
        ...
        toAttrs v
          = catMaybes [maybeToAttr toAttrFrStr "colspan" (cColspan v)]
...
instance XmlAttributes Box_Attrs where
        fromAttrs as
          = Box_Attrs{boxHibit = definiteA fromAttrToStr "box" "hibit" as,
        ...
        toContents (Encoding as a b c d e)
          = [CElem
               (Elem (N "encoding") (toAttrs as)
                  (maybe [] toContents a ++
                     maybe [] toContents b ++
                       concatMap toContents c ++ toContents d ++ maybe [] toContents e))
               ()]
        parseContents
...
instance XmlAttributes Classesintro_Attrs where
        fromAttrs as
          = Classesintro_Attrs{classesintroCount =
                                 possibleA fromAttrToStr "count" as}
        toAttrs v
          = catMaybes [maybeToAttr toAttrFrStr "count" (classesintroCount v)]
...
instance XmlContent Classes where
        toContents (Classes a b)
          = [CElem
               (Elem (N "classes") [] (maybe [] toContents a ++ toContents b))
               ()]
        parseContents
          = do e@(Elem _ [] _) <- element ["classes"]
               interior e $
                 return (Classes) `apply` optional parseContents `apply`
                   parseContents
...
instance XmlAttributes Iclassintro_Attrs where
        fromAttrs as
          = Iclassintro_Attrs{iclassintroCount =
                                possibleA fromAttrToStr "count" as}
        toAttrs v
          = catMaybes [maybeToAttr toAttrFrStr "count" (iclassintroCount v)]
               ...
               (Elem (N "iclass") (toAttrs as)
                  (maybe [] toContents a ++
                     maybe [] toContents b ++
                       maybe [] toContents c ++
                         toContents d ++
                           toContents e ++ maybe [] toContents f ++ maybe [] toContents g))
               ()]
        parseContents
...
instance XmlContent Explanations where
        toContents (Explanations as a)
          = [CElem
               (Elem (N "explanations") (toAttrs as) (concatMap toContents a))
               ()]
        parseContents
          = do e@(Elem _ as _) <- element ["explanations"]
               interior e $
                 return (Explanations (fromAttrs as)) `apply` many parseContents
              `adjustErr` ("in <explanations>, " ++)

instance XmlAttributes Explanations_Attrs where
        fromAttrs as
          = Explanations_Attrs{explanationsScope =
                                 definiteA fromAttrToTyp "explanations" "scope" as}
        toAttrs v = catMaybes [toAttrFrTyp "scope" (explanationsScope v)]
...
instance XmlAttributes Explanation_Attrs where
        fromAttrs as
          = Explanation_Attrs{explanationEnclist =
                                definiteA fromAttrToStr "explanation" "enclist" as,
                              explanationTags = possibleA fromAttrToStr "tags" as,
                              explanationSymboldefcount =
                                definiteA fromAttrToStr "explanation" "symboldefcount" as}
        toAttrs v
          = catMaybes
...
instance XmlAttributes Table_Attrs where
        fromAttrs as
          = Table_Attrs{tableClass =
                          definiteA fromAttrToTyp "table" "class" as}
        toAttrs v = catMaybes [toAttrFrTyp "class" (tableClass v)]
...
instance XmlAttrType Table_class where
        fromAttrToTyp n (N n', v)
          ...
          where translate "valuetable" = Just Table_class_valuetable
                translate _ = Nothing
        toAttrFrTyp n Table_class_valuetable
          = Just (N n, str2attr "valuetable")
...
instance XmlAttributes Entry_Attrs where
        fromAttrs as
          = Entry_Attrs{entryClass =
                          definiteA fromAttrToTyp "entry" "class" as,
...
instance XmlAttrType Entry_class where
        ...
        toAttrFrTyp n Entry_class_bitfield
          = Just (N n, str2attr "bitfield")
...
instance XmlContent Constrained_unpredictables where
        toContents (Constrained_unpredictables as a)
          = [CElem
               (Elem (N "constrained_unpredictables") (toAttrs as) (toContents a))
               ()]
        parseContents
          = do e@(Elem _ as _) <- element ["constrained_unpredictables"]
               interior e $
                 return (Constrained_unpredictables (fromAttrs as)) `apply`
                   parseContents
              `adjustErr` ("in <constrained_unpredictables>, " ++)

instance XmlAttributes Constrained_unpredictables_Attrs where
        fromAttrs as
          = Constrained_unpredictables_Attrs{constrained_unpredictablesEncoding
                                               = possibleA fromAttrToStr "encoding" as,
                                             constrained_unpredictablesPs_block =
                                               possibleA fromAttrToStr "ps_block" as}
        toAttrs v
          = catMaybes
              [maybeToAttr toAttrFrStr "encoding"
                 (constrained_unpredictablesEncoding v),
               maybeToAttr toAttrFrStr "ps_block"
                 (constrained_unpredictablesPs_block v)]
...
instance XmlContent Cu_case where
        toContents (Cu_case a b c)
          = [CElem
               ...
               ()]
        parseContents
          = do e@(Elem _ [] _) <- element ["cu_case"]
               interior e $
                 return (Cu_case) `apply` parseContents `apply` parseContents
                   `apply` optional parseContents
...
instance XmlContent Cu_cause where
        ...
        parseContents
          = do e@(Elem _ [] _) <- element ["cu_cause"]
...
instance XmlContent Cu_type where
        toContents (Cu_type as a b)
          = [CElem
               (Elem (N "cu_type") (toAttrs as)
                  (concatMap toContents a ++ concatMap toContents b))
               ()]
        parseContents
          = do e@(Elem _ as _) <- element ["cu_type"]
               interior e $
                 return (Cu_type (fromAttrs as)) `apply` many parseContents `apply`
                   many parseContents
              `adjustErr` ("in <cu_type>, " ++)

instance XmlAttributes Cu_type_Attrs where
        fromAttrs as
          = Cu_type_Attrs{cu_typeConstraint =
                            possibleA fromAttrToStr "constraint" as}
        toAttrs v
          = catMaybes
...
instance XmlContent Cu_type_variable where
        ...
        parseContents
          = do (Elem _ as []) <- element ["cu_type_variable"]
               return (fromAttrs as)
              `adjustErr` ("in <cu_type_variable>, " ++)

instance XmlAttributes Cu_type_variable where
        fromAttrs as
          = Cu_type_variable{cu_type_variableName =
                               definiteA fromAttrToStr "cu_type_variable" "name" as,
                             cu_type_variableValue =
                               definiteA fromAttrToStr "cu_type_variable" "value" as}
        toAttrs v
          = catMaybes
...
instance XmlContent Image where
        toContents as = [CElem (Elem (N "image") (toAttrs as) []) ()]
        parseContents
          = do (Elem _ as []) <- element ["image"]
               return (fromAttrs as)
              `adjustErr` ("in <image>, " ++)

instance XmlAttributes Image where
        fromAttrs as
          = Image{imageFile = definiteA fromAttrToStr "image" "file" as,
