...
data Register_links = Register_links Register_links_Attrs
                                     [Register_link]
                    deriving (Eq,Show)
data Register_links_Attrs = Register_links_Attrs
    { register_linksTitle :: String
    } deriving (Eq,Show)
...
instance XmlAttributes Toptitle where
    fromAttrs as =
        Toptitle
          { toptitleArchitecture = definiteA fromAttrToStr "toptitle" "architecture" as
          }
    toAttrs v = catMaybes 
        [ toAttrFrStr "architecture" (toptitleArchitecture v)
        ]
...
instance XmlContent Register_index where
    toContents (Register_index a b c) =
        [CElem (Elem (N "register_index") [] (toContents a ++ toContents b
                                              ++ toContents c)) ()]
    parseContents = do
        { e@(Elem _ [] _) <- element ["register_index"]
        ; interior e $ return (Register_index) `apply` parseContents
...
instance XmlContent Register_links where
    toContents (Register_links as a) =
        [CElem (Elem (N "register_links") (toAttrs as) (concatMap toContents a)) ()]
    parseContents = do
        { e@(Elem _ as _) <- element ["register_links"]
        ; interior e $ return (Register_links (fromAttrs as))
                       `apply` many parseContents
        } `adjustErr` ("in <register_links>, "++)
instance XmlAttributes Register_links_Attrs where
    fromAttrs as =
        Register_links_Attrs
          { register_linksTitle = definiteA fromAttrToStr "register_links" "title" as
          }
    toAttrs v = catMaybes 
        [ toAttrFrStr "title" (register_linksTitle v)
        ]
...
instance XmlAttributes Register_link_Attrs where
    fromAttrs as =
        Register_link_Attrs
          { register_linkId = definiteA fromAttrToStr "register_link" "id" as
          , register_linkHeading = definiteA fromAttrToStr "register_link" "heading" as
          , register_linkRegisterfile = definiteA fromAttrToStr "register_link" "registerfile" as
          }
    toAttrs v = catMaybes 
