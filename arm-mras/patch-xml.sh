...
for f in $(find $root -name 'mov_rr.xml'); do
  sed -i 's|width=""|width="4"|' $f
done

for f in $(find $root -name 'vcvt_ds.xml'); do
  sed -i 's|width=""|width="2"|' $f
done

for f in $(find $root -name 'vcvt_iv.xml'); do
  sed -i 's|width=""|width="3"|' $f
done

for f in $(find $root -name 'vcvtr_iv.xml'); do
  sed -i 's|width=""|width="3"|' $f
done

for f in $(find $root -name 'vmov_r.xml'); do
  sed -i 's|width=""|width="2"|' $f
done
