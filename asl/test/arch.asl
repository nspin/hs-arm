...
enumeration Constraint    {// General:
                           Constraint_NONE,              // Instruction executes with
                                                         //  no change or side-effect to its described behaviour
...
AArch32.CheckAdvSIMDOrFPEnabled(boolean fpexc_check, boolean advsimd)
    if PSTATE.EL == EL0 && (!HaveEL(EL2) || (!ELUsingAArch32(EL2) && HCR_EL2.TGE == '0')) && !ELUsingAArch32(EL1) then
        ...
        AArch64.CheckFPAdvSIMDEnabled();
    elsif PSTATE.EL == EL0 && HaveEL(EL2) && !ELUsingAArch32(EL2) && HCR_EL2.TGE == '1' then
...
AArch64.CheckForSMCUndefOrTrap(bits(16) imm)
    ...
    if !HaveEL(EL3) then
        ...
        route_to_el2 = HaveEL(EL2) && !IsSecure() && PSTATE.EL == EL1 && HCR_EL2.TSC == '1';
    if route_to_el2 then
        bits(64) preferred_exception_return = ThisInstrAddr();
        vect_offset = 0x0;
...
TLBRecord AArch64.TranslationTableWalk(bits(52) ipaddress, bits(64) vaddress,
    ...
    apply_nvnv1_effect = HaveNVExt() && HaveEL(EL2) && HCR_EL2.NV == 1 && HCR_EL2.NV1 == 1;
    ...
    sh = desc<9:8>;
    if apply_nvnv1_effect then
        ap = desc<7>:'01';                                    // Bit[6] is treated as 0 regardless of value programmed
    else
        ap = desc<7:6>:'1';
...
TLBRecord AArch64.TranslateAddressS1Off(bits(64) vaddress, AccType acctype, boolean iswrite)
    ...
    Top = AddrTop(vaddress, FALSE, PSTATE.EL);
...
boolean AArch64.AccessIsPrivileged(AccType acctype)
    ...
    if PSTATE.EL == EL0 then
        ...
        ispriv = TRUE;
    elsif PSTATE.EL == EL2 && (!IsInHost() || HCR_EL2.TGE == '0') then
        ispriv = TRUE;
...
FaultRecord AArch64.CheckPermission(Permissions perms, bits(64) vaddress, integer level,
    ...
    if PSTATE.EL IN {EL0,EL1} || IsInHost() then
        ...
        if (HaveNVExt() && HaveEL(EL2) && HCR_EL2.NV == 1 && HCR_EL2.NV1 == 1 && !IsSecure() && PSTATE.EL == EL1) then
            pan = '0';
        if (pan == '1' && user_r && ispriv &&
...
AArch64.CheckAArch32SystemAccessEL2Traps(bits(32) instr)
    assert HaveEL(EL2) && !IsSecure() && PSTATE.EL IN {EL0, EL1, EL2};
    ...
    if cp_num == 14 && PSTATE.EL IN {EL0, EL1} then
    ...
    elsif cp_num == 14 && PSTATE.EL == EL2 then
        if opc1 == 1 then
            trap = CPTR_EL2.TTA == '1';

    elsif cp_num == 15 && PSTATE.EL IN {EL0, EL1} then
...
AArch64.CheckAArch32SystemAccessTraps(bits(32) instr)
    ...
    if HaveEL(EL2) && !IsSecure() && PSTATE.EL IN {EL0, EL1, EL2} && !IsInHost() then
...
bits(N) Sat(integer i, integer N, boolean unsigned)
    ...
    return result;
// IsEL1TransRegimeRegs()
// ======================

// Returns TRUE if its a register in EL1 translation regime

boolean IsEL1TransRegimeRegs()
    return !HaveEL(EL2) || PSTATE.EL == EL1 || (PSTATE.EL == EL0 && (HCR_EL2.E2H == '0' ||  HCR_EL2.TGE=='0'));
...
AArch64.CheckForERetTrap(boolean eret_with_pac, boolean pac_uses_key_a)
    ...
    route_to_el2 = (HaveNVExt() && HaveEL(EL2) && !IsSecure() && PSTATE.EL == EL1 && HCR_EL2.NV == '1');
    ...
    if route_to_el2 then
        ...
        exception.syndrome<24:2> = ZeroExtend('0', 23);    // syndrome<24:2> is RES0
...
integer CalculateBottomPACBit(bits(64) ptr, bit top_bit)
    integer tsz_field;

    if PtrHasUpperAndLowerAddRanges() then
        if IsEL1TransRegimeRegs() then
            tsz_field = if top_bit == '1' then UInt(TCR_EL1.T1SZ) else UInt(TCR_EL1.T0SZ);
            using64k = if top_bit == '1' then TCR_EL1.TG1 == '11' else TCR_EL1.TG0 == '11';
        else
            // EL2 translation regime registers
            assert HaveEL(EL2);
            tsz_field = if top_bit == '1' then UInt(TCR_EL2.T1SZ) else UInt(TCR_EL2.T0SZ);
            using64k = if top_bit == '1' then TCR_EL2.TG1 == '11' else TCR_EL2.TG0 == '11';
    else
        tsz_field = if PSTATE.EL == EL2 then UInt(TCR_EL2.T0SZ) else UInt(TCR_EL3.T0SZ);
        using64k = if PSTATE.EL == EL2 then TCR_EL2.TG0 == '11' else TCR_EL3.TG0 == '11';

    max_limit_tsz_field = 39;
...
bits(64) AddPAC(bits(64) ptr, bits(64) modifier, bits(128) K, boolean data)
    ...
    integer bottom_PAC_bit = CalculateBottomPACBit(ptr, selbit);
...
bits(64) AddPACDA(bits(64) X, bits(64) Y)
    ...
    case PSTATE.EL of
        when EL0
            boolean IsEL1Regime = !HaveEL(EL2) || HCR_EL2.TGE=='0' || HCR_EL2.E2H=='0';
            Enable = if IsEL1Regime then SCTLR_EL1.EnDA else SCTLR_EL2.EnDA;
            TrapEL2 = HaveEL(EL2) && IsEL1Regime && !IsSecure() && HCR_EL2.API == '0';
            TrapEL3 = HaveEL(EL3) && SCR_EL3.API == '0';
        when EL1
            Enable = SCTLR_EL1.EnDA;
            TrapEL2 = HaveEL(EL2) && !IsSecure() && HCR_EL2.API == '0';
            TrapEL3 = HaveEL(EL3) && SCR_EL3.API == '0';
        when EL2
...
bits(64) Auth(bits(64) ptr, bits(64) modifier, bits(128) K, boolean data, bit keynumber)
    ...
    boolean tbi = CalculateTBI(ptr, data);
    integer bottom_PAC_bit = CalculateBottomPACBit(ptr, ptr<55>);
...
bits(64) Strip(bits(64) A, boolean data)
    ...
    integer bottom_PAC_bit = CalculateBottomPACBit(A, A<55>);
...
AArch64.CheckSystemAccess(bits(2) op0, bits(3) op1, bits(4) crn, bits(4) crm, bits(3) op2, bits(5) rt, bit read)
    ...
    if UInt(PSTATE.EL) < UInt(min_EL) then
        ...
        if ( PSTATE.EL == EL1 && min_EL == EL2 && HaveNVExt() && !IsSecure() && HaveEL(EL2) && HCR_EL2.NV == '1' ) then
            AArch64.SystemRegisterTrap(EL2, op0, op2, op1, crn, rt, crm, read);
        else
            UnallocatedEncoding();
...
// accessable by reads, writes or execution, as defined in the translation regime of the current
...
bits(32) PMPCSR[boolean memory_mapped]
    ...
    if pc_sample.valid then
        ...
        if update then
            ...
            PMCID2SR = (if HaveEL(EL2) && pc_sample.ns == '1' then pc_sample.contextidr_el2
                        else bits(32) UNKNOWN);

            PMVIDSR.VMID = (if HaveEL(EL2) && pc_sample.ns == '1' && pc_sample.el IN {EL1,EL0}
...
// HaveChCryptoExt()
// =================
// TRUE if SM3 and SM4 extended cryptographic instructions support is implemented,
// FALSE otherwise.

boolean HaveChCryptoExt()
    if !HasArchVersion(ARMv8p2) then
        return FALSE;
    return boolean IMPLEMENTATION_DEFINED "Has SM3 and SM4 Crypto instructions";
// HaveCryptoExt()
// ===============
// TRUE if AES, SHA1, SHA2 cryptographic instructions support is implemented,
// FALSE otherwise.

boolean HaveCryptoExt()
    return boolean IMPLEMENTATION_DEFINED "Has basic Crypto instructions";
// HaveCryptoExt2()
// ================
// TRUE if SHA512 and SHA3 extended cryptographic instructions support is implemented,
// and when SHA1 and SHA2 basic cryptographic instructions support is implemented,
// FALSE otherwise.

boolean HaveCryptoExt2()
    if !HasArchVersion(ARMv8p2) || !HaveCryptoExt() then
        return FALSE;
    return boolean IMPLEMENTATION_DEFINED "Has SHA512 and SHA3 Crypto instructions";
// ROL()
// =====
...
boolean HaveDOTPExt()
    return HasArchVersion(ARMv8p2) && boolean IMPLEMENTATION_DEFINED "Has Dot Product extension";
// HaveFCADDExt()
// ==============
