...
data Sysregindex = Sysregindex Sysregindex_Attrs Typelist
                 ...
                 deriving (Eq,Show)
data Sysregindex_Attrs = Sysregindex_Attrs
    { sysregindexIndextype :: Sysregindex_indextype
    , sysregindexIs_internal :: (Maybe Sysregindex_is_internal)
    } deriving (Eq,Show)
...
newtype Typelist = Typelist (List1 Typegroup) 		deriving (Eq,Show)

data Typegroup = Typegroup Typegroup_Attrs [Typelink]
               deriving (Eq,Show)
data Typegroup_Attrs = Typegroup_Attrs
    { typegroupGroupname :: String
    } deriving (Eq,Show)

data Typelink = Typelink
    { typelinkType :: String
    , typelinkLink :: String
    } deriving (Eq,Show)
...
data Sectiongroup = Sectiongroup Sectiongroup_Attrs [Section]
                  deriving (Eq,Show)
data Sectiongroup_Attrs = Sectiongroup_Attrs
    { sectiongroupGroupname :: String
    } deriving (Eq,Show)
...
newtype Heading = Heading (List1 Row) 		deriving (Eq,Show)
...
data Entry_Attrs = Entry_Attrs
    { entryRowspan :: (Maybe String)
    , entryColspan :: (Maybe String)
...
instance XmlContent Sysregindex where
    toContents (Sysregindex as a b c) =
        [CElem (Elem (N "sysregindex") (toAttrs as) (toContents a ++
                                                     toContents b ++ toContents c)) ()]
    parseContents = do
        { e@(Elem _ as _) <- element ["sysregindex"]
        ; interior e $ return (Sysregindex (fromAttrs as))
...
instance XmlAttributes Sysregindex_Attrs where
    fromAttrs as =
        Sysregindex_Attrs
          { sysregindexIndextype = definiteA fromAttrToTyp "sysregindex" "indextype" as
...
instance XmlAttrType Sysregindex_indextype where
    fromAttrToTyp n (N n',v)
      ...
      where translate "encoding" = Just Sysregindex_indextype_encoding
            ...
            translate _ = Nothing
    toAttrFrTyp n Sysregindex_indextype_encoding = Just (N n, str2attr "encoding")
    toAttrFrTyp n Sysregindex_indextype_function = Just (N n, str2attr "function")
...
instance XmlAttrType Sysregindex_is_internal where
    fromAttrToTyp n (N n',v)
      ...
      where translate "True" = Just Sysregindex_is_internal_True
            ...
            translate _ = Nothing
    toAttrFrTyp n Sysregindex_is_internal_True = Just (N n, str2attr "True")
    toAttrFrTyp n Sysregindex_is_internal_False = Just (N n, str2attr "False")
...
instance XmlContent Typegroup where
    toContents (Typegroup as a) =
        [CElem (Elem (N "typegroup") (toAttrs as) (concatMap toContents a)) ()]
    parseContents = do
        { e@(Elem _ as _) <- element ["typegroup"]
        ; interior e $ return (Typegroup (fromAttrs as))
                       `apply` many parseContents
        } `adjustErr` ("in <typegroup>, "++)
instance XmlAttributes Typegroup_Attrs where
    fromAttrs as =
        Typegroup_Attrs
          { typegroupGroupname = definiteA fromAttrToStr "typegroup" "groupname" as
          }
    toAttrs v = catMaybes 
        [ toAttrFrStr "groupname" (typegroupGroupname v)
        ]
...
instance XmlContent Typelink where
    toContents as =
        [CElem (Elem (N "typelink") (toAttrs as) []) ()]
    parseContents = do
        { (Elem _ as []) <- element ["typelink"]
        ; return (fromAttrs as)
        } `adjustErr` ("in <typelink>, "++)
instance XmlAttributes Typelink where
    fromAttrs as =
        Typelink
          { typelinkType = definiteA fromAttrToStr "typelink" "type" as
...
instance XmlContent Sectiongroup where
    toContents (Sectiongroup as a) =
        [CElem (Elem (N "sectiongroup") (toAttrs as) (concatMap toContents a)) ()]
    parseContents = do
        { e@(Elem _ as _) <- element ["sectiongroup"]
        ; interior e $ return (Sectiongroup (fromAttrs as))
                       `apply` many parseContents
        } `adjustErr` ("in <sectiongroup>, "++)
instance XmlAttributes Sectiongroup_Attrs where
    fromAttrs as =
        Sectiongroup_Attrs
          { sectiongroupGroupname = definiteA fromAttrToStr "sectiongroup" "groupname" as
          }
    toAttrs v = catMaybes 
        [ toAttrFrStr "groupname" (sectiongroupGroupname v)
        ]
...
instance XmlContent Section where
    toContents (Section as a b) =
        [CElem (Elem (N "section") (toAttrs as) (toContents a ++
                                                 toContents b)) ()]
    parseContents = do
        { e@(Elem _ as _) <- element ["section"]
        ; interior e $ return (Section (fromAttrs as))
                       `apply` parseContents `apply` parseContents
        } `adjustErr` ("in <section>, "++)
instance XmlAttributes Section_Attrs where
    fromAttrs as =
        Section_Attrs
          { sectionType = definiteA fromAttrToStr "section" "type" as
...
instance XmlContent Row where
    ...
    parseContents = do
        { e@(Elem _ as _) <- element ["row"]
        ; interior e $ return (Row (fromAttrs as)) `apply` parseContents
        } `adjustErr` ("in <row>, "++)
instance XmlAttributes Row_Attrs where
    fromAttrs as =
        Row_Attrs
          { rowClass = possibleA fromAttrToStr "class" as
          }
    toAttrs v = catMaybes 
        [ maybeToAttr toAttrFrStr "class" (rowClass v)
        ]
...
instance XmlAttributes Entry_Attrs where
    fromAttrs as =
        Entry_Attrs
          { entryRowspan = possibleA fromAttrToStr "rowspan" as
