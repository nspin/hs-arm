...
newtype Access_instruction_defs = Access_instruction_defs (List1 Access_instruction_def) 		deriving (Eq,Show)
...
data Access_instruction_def = Access_instruction_def Access_instruction_def_Attrs
                            ...
                            deriving (Eq,Show)
data Access_instruction_def_Attrs = Access_instruction_def_Attrs
    { access_instruction_defExecution_state :: Access_instruction_def_execution_state
...
data Access_type = Access_type
    { access_typeType :: Access_type_type
    } deriving (Eq,Show)

data Access_type_type = Access_type_type_read  | 
                        Access_type_type_write  |  Access_type_type_modify
                      deriving (Eq,Show)

data Access_syntax = Access_syntax Access_syntax_Attrs [Var]
                   deriving (Eq,Show)
data Access_syntax_Attrs = Access_syntax_Attrs
    { access_syntaxName :: String
...
instance XmlContent Access_instruction_defs where
    ...
    parseContents = do
        { e@(Elem _ [] _) <- element ["access_instruction_defs"]
        ; interior e $ return (Access_instruction_defs)
                       `apply` parseContents
...
instance XmlContent Access_instruction_def where
    toContents (Access_instruction_def as a b c) =
        [CElem (Elem (N "access_instruction_def") (toAttrs as) (toContents a
                                                                ++ maybe [] toContents b ++
                                                                toContents c)) ()]
    parseContents = do
        { e@(Elem _ as _) <- element ["access_instruction_def"]
        ; interior e $ return (Access_instruction_def (fromAttrs as))
...
instance XmlAttributes Access_instruction_def_Attrs where
    fromAttrs as =
        Access_instruction_def_Attrs
          { access_instruction_defExecution_state = definiteA fromAttrToTyp "access_instruction_def" "execution_state" as
          , access_instruction_defId = definiteA fromAttrToStr "access_instruction_def" "id" as
          }
    toAttrs v = catMaybes 
        [ toAttrFrTyp "execution_state" (access_instruction_defExecution_state v)
...
instance XmlAttrType Access_instruction_def_execution_state where
    fromAttrToTyp n (N n',v)
        ...
        | otherwise = Nothing
      where translate "AArch32" = Just Access_instruction_def_execution_state_AArch32
            translate "AArch64" = Just Access_instruction_def_execution_state_AArch64
            translate _ = Nothing
    toAttrFrTyp n Access_instruction_def_execution_state_AArch32 = Just (N n, str2attr "AArch32")
    toAttrFrTyp n Access_instruction_def_execution_state_AArch64 = Just (N n, str2attr "AArch64")
...
instance XmlContent Access_type where
    toContents as =
        [CElem (Elem (N "access_type") (toAttrs as) []) ()]
    parseContents = do
        { (Elem _ as []) <- element ["access_type"]
        ; return (fromAttrs as)
        } `adjustErr` ("in <access_type>, "++)
instance XmlAttributes Access_type where
    fromAttrs as =
        Access_type
          { access_typeType = definiteA fromAttrToTyp "access_type" "type" as
          }
    toAttrs v = catMaybes 
        [ toAttrFrTyp "type" (access_typeType v)
        ]
...
instance XmlAttrType Access_type_type where
    ...
    toAttrFrTyp n Access_type_type_modify = Just (N n, str2attr "modify")
...
instance XmlContent Access_syntax where
    toContents (Access_syntax as a) =
        [CElem (Elem (N "access_syntax") (toAttrs as) (concatMap toContents a)) ()]
    parseContents = do
        { e@(Elem _ as _) <- element ["access_syntax"]
        ; interior e $ return (Access_syntax (fromAttrs as))
                       `apply` many parseContents
        } `adjustErr` ("in <access_syntax>, "++)
instance XmlAttributes Access_syntax_Attrs where
    fromAttrs as =
        Access_syntax_Attrs
          { access_syntaxName = definiteA fromAttrToStr "access_syntax" "name" as
...
instance XmlContent Var where
    ...
    parseContents = do
        ...
        ; interior e $ return (Var (fromAttrs as))
                       `apply` many parseContents
        } `adjustErr` ("in <var>, "++)
instance XmlAttributes Var_Attrs where
    fromAttrs as =
        Var_Attrs
          { varN = definiteA fromAttrToStr "var" "n" as
          , varT = possibleA fromAttrToStr "t" as
          , varPrefix = possibleA fromAttrToStr "prefix" as
          }
    toAttrs v = catMaybes 
        [ toAttrFrStr "n" (varN v)
        , maybeToAttr toAttrFrStr "t" (varT v)
