module ARM.MRAS.DTD.AArch32.Encodingindex where
...
data Tr_Attrs = Tr_Attrs
    ...
    , trGroupname :: (Maybe String)
    , trArch_version :: (Maybe String)
    } deriving (Eq,Show)

data Tr_size = Tr_size_16  |  Tr_size_16x2  |  Tr_size_32
             deriving (Eq,Show)
...
newtype Tablebody = Tablebody (List1 Tablebody_) 		deriving (Eq,Show)
...
newtype Tbody = Tbody (List1 Tr) 		deriving (Eq,Show)
...
newtype Header = Header String 		deriving (Eq,Show)

newtype Decode = Decode (List1 Box) 		deriving (Eq,Show)

newtype Decode_constraints = Decode_constraints (List1 Decode_constraint) 		deriving (Eq,Show)

data Decode_constraint = Decode_constraint
    { decode_constraintName :: String
...
data Node_unallocated = Node_unallocated_0  |  Node_unallocated_1
                      deriving (Eq,Show)
...
data Mapping = Mapping
    { mappingFrom_isa :: (Maybe String)
    , mappingFrom :: String
    , mappingTo_isa :: String
    , mappingTo :: String
    } deriving (Eq,Show)

newtype Mappings = Mappings [Mapping] 		deriving (Eq,Show)
...
data Encodingindex_Attrs = Encodingindex_Attrs
    { encodingindexInstructionset :: (Maybe String)
...
instance XmlAttributes Tr_Attrs where
    fromAttrs as =
        Tr_Attrs
          ...
          , trReserved_nop_hint = possibleA fromAttrToTyp "reserved_nop_hint" as
    ...
    toAttrs v = catMaybes 
        ...
        , maybeToAttr toAttrFrTyp "reserved_nop_hint" (trReserved_nop_hint v)
...
instance XmlContent Instructiontable where
    toContents (Instructiontable as a b c) =
        [CElem (Elem (N "instructiontable") (toAttrs as) (concatMap toContents a
                                                          ++ toContents b ++ toContents c)) ()]
    parseContents = do
...
instance XmlContent Decode_constraints where
    ...
    parseContents = do
        { e@(Elem _ [] _) <- element ["decode_constraints"]
...
instance XmlContent Decode_constraint where
    ...
    parseContents = do
        { (Elem _ as []) <- element ["decode_constraint"]
        ; return (fromAttrs as)
        } `adjustErr` ("in <decode_constraint>, "++)
instance XmlAttributes Decode_constraint where
    fromAttrs as =
        Decode_constraint
          { decode_constraintName = definiteA fromAttrToStr "decode_constraint" "name" as
          , decode_constraintOp = definiteA fromAttrToStr "decode_constraint" "op" as
          , decode_constraintVal = definiteA fromAttrToStr "decode_constraint" "val" as
          }
    toAttrs v = catMaybes 
...
instance XmlAttributes Node_Attrs where
    fromAttrs as =
        Node_Attrs
          { nodeIclass = possibleA fromAttrToStr "iclass" as
          ...
          , nodeReserved_nop_hint = possibleA fromAttrToTyp "reserved_nop_hint" as
          }
    toAttrs v = catMaybes 
        ...
        , maybeToAttr toAttrFrTyp "reserved_nop_hint" (nodeReserved_nop_hint v)
        ]
...
instance XmlContent Hierarchy where
    toContents (Hierarchy a b) =
        [CElem (Elem (N "hierarchy") [] (toContents a ++
                                         concatMap toContents b)) ()]
    parseContents = do
        { e@(Elem _ [] _) <- element ["hierarchy"]
        ; interior e $ return (Hierarchy) `apply` parseContents
                       `apply` many parseContents
...
instance XmlContent Mapping where
    toContents as =
        [CElem (Elem (N "mapping") (toAttrs as) []) ()]
    parseContents = do
        { (Elem _ as []) <- element ["mapping"]
        ; return (fromAttrs as)
        } `adjustErr` ("in <mapping>, "++)
instance XmlAttributes Mapping where
    fromAttrs as =
        Mapping
          { mappingFrom_isa = possibleA fromAttrToStr "from_isa" as
...
instance XmlAttributes Encodingindex_Attrs where
    fromAttrs as =
        Encodingindex_Attrs
          { encodingindexInstructionset = possibleA fromAttrToStr "instructionset" as
          , encodingindexTitle = possibleA fromAttrToStr "title" as
          }
    toAttrs v = catMaybes 
        [ maybeToAttr toAttrFrStr "instructionset" (encodingindexInstructionset v)
